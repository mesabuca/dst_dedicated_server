...
GLOBAL.TUNING.AMULETE_SLOT = GetModConfigData("amulete_slot")
...
AddGlobalClassPostConstruct("widgets/inventorybar", "Inv", function()
    ...
    function Inv:LoadExtraSlots(self)
        ...
        if self.addextraslots == nil then
            ...
            if GLOBAL.TUNING.AMULETE_SLOT == 1 then
...
function amuletpostinit(inst)
    if IsServer and GLOBAL.TUNING.AMULETE_SLOT == 1 then
...
if TUNING.COMPASS_STATE == 1 then
    local function TryCompass(self)
        if self.owner.replica.inventory ~= nil then
            local equipment = self.owner.replica.inventory:GetEquippedItem(GLOBAL.EQUIPSLOTS.WAIST or GLOBAL.EQUIPSLOTS.HANDS)
    ...
    local function hudcompasspostconstruct(self)
        local listeners = self.owner.event_listeners["refreshinventory"][self.inst]
        local OldTryCompass = listeners[#listeners]
        self.inst:RemoveEventCallback("refreshinventory", OldTryCompass, self.owner)
